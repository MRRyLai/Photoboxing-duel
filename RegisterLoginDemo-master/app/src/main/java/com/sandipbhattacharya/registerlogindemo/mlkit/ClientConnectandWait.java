...
public class ClientConnectandWait extends AppCompatActivity {
    ...
    private String message,get_rev,put_send,ClientConnectandWiait_opposite_name,ClientConnectandWiait_name="",ClientConnectandWiait_opposite_email="";
    private String[] info;
...
//宣告一個緩衝,從br串流讀取 Server 端傳來的訊息
                    get_rev = br.readLine();
                    info = get_rev.split(", ");//info[0]:ready
                    Log.v("ClientConnect&Wait_tcp","GetAllmsg"+get_rev);
                    if(get_rev!=null){
                        //將取到的String抓取{}範圍資料
                        Log.v("ClientConnect&Wait_tcp","revive:"+get_rev);
                        ClientConnectandWiait_opposite_name = info[0];
                        ClientConnectandWiait_opposite_email = info[1];
                        get_rev=null;

                        if(ClientConnectandWiait_opposite_score>=0){
                            Intent intent = new Intent(ClientConnectandWait.this, CameraXLivePreviewActivity.class);
